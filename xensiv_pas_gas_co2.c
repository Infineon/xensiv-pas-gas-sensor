...
int32_t xensiv_pas_gas_co2_cmd(const xensiv_pas_gas_t *dev, void *cmd) {
    xensiv_pas_gas_co2_cmd_t co2_cmd = *(xensiv_pas_gas_co2_cmd_t *)cmd;
    xensiv_pas_gas_plat_assert(dev != NULL);

    return xensiv_pas_gas_set_reg(dev, (uint8_t)XENSIV_PAS_GAS_REG_SENS_RST, (const uint8_t *)&co2_cmd, 1U);
}

int32_t xensiv_pas_gas_co2_start_single_mode(const xensiv_pas_gas_t *dev) {
    xensiv_pas_gas_plat_assert(dev != NULL);

    xensiv_pas_gas_co2_measurement_config_t meas_config;
...
int32_t xensiv_pas_gas_co2_start_continuous_mode(const xensiv_pas_gas_t *dev, uint16_t val) {
    xensiv_pas_gas_plat_assert(dev != NULL);
...
int32_t xensiv_pas_gas_co2_set_measurement_rate(const xensiv_pas_gas_t *dev, uint16_t val) {
    xensiv_pas_gas_plat_assert(dev != NULL);
    xensiv_pas_gas_plat_assert((val >= XENSIV_PAS_GAS_CO2_MEAS_RATE_MIN) && (val <= XENSIV_PAS_GAS_MEAS_RATE_MAX));
...
int32_t xensiv_pas_gas_co2_perform_forced_compensation(const xensiv_pas_gas_t *dev, uint16_t gas_ref) {
    xensiv_pas_gas_plat_assert(dev != NULL);
    ...
    if (XENSIV_PAS_GAS_OK == res) {
        res = xensiv_pas_gas_co2_set_measurement_rate(dev, XENSIV_PAS_GAS_FCS_MEAS_RATE_S);
    }
    ...
    if (XENSIV_PAS_GAS_OK == res) {
        res = xensiv_pas_gas_cmd(dev, (void *)(uintptr_t)XENSIV_PAS_GAS_CO2_CMD_SAVE_FCS_CALIB_OFFSET);
    }
